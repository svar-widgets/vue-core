<script setup>
defineOptions({ name: 'CoreSegmented', inheritAttrs: false });
...
<template>
  <div class="wx-segmented" :class="props.css">
    <button
    ...
    >
      ...
      <template v-else>
        <i
          v-if="option.icon"
          class="wx-icon"
          :class="[option.icon, !option.label ? 'wx-only' : '']"
        ></i>
        <span v-if="option.label" class="wx-label">{{ option.label }}</span>
      </template>
    </button>
  </div>
</template>
