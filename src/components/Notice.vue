...
function onRemove() {
  if (props.notice && props.notice.remove) props.notice.remove();
}
</script>

<template>
  <Transition name="wx-notice-fade" appear>
    <div
      class="wx-notice"
      :class="`wx-${props.notice.type ? props.notice.type : ''}`"
      role="status"
      aria-live="polite"
    >
      <div class="wx-text">{{ props.notice.text }}</div>
      <div class="wx-button">
        <i class="wxi-close" @click="onRemove"></i>
      </div>
    </div>
  </Transition>
</template>
...
<style>
.wx-notice-fade-enter-from,
.wx-notice-fade-leave-to {
  opacity: 0;
}
.wx-notice-fade-enter-active,
.wx-notice-fade-leave-active {
